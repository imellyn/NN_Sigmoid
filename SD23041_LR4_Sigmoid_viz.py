...
st.set_page_config(page_title = "Sigmoid Neural Network",
                   layout ="wide")

st.title("Neural Network Classification with Sigmoid Activation")
st.write(
    """
    This app trains a PyTorch MLP with Sigmoid activation on synthetic 2D data.
    Features are standardized (fit on train only). Visualizations include:
    - Activation curve
    - Dataset
    - Loss curve
    - Accuracy
...
#Neural Network Model with Sigmoid 
class MLP(nn.Module):
...
if st.button("Train Model"):
    ...
    ax.set_title("Decision Boundary (Sigmoid)")
    st.pyplot(fig)
